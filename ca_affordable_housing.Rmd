...
affordable_units_in_neighborhood <- 
  ...
  group_by(nhood) %>% 
  summarize(affordable_units = sum(affordable_units)) %>%
  ungroup()
...
```{r}
View(affordable_housing_sf_joined %>% 
  filter(city_analysis_neighborhood == "South of Market") %>% 
  select(c(project_name, city_analysis_neighborhood, nhood)))

```

```{r}
unique(affordable_housing_sf$city_analysis_neighborhood)
```
